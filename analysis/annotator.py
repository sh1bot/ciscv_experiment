...
class PairingStats:
    pairs:     int = 0
    solos:     int = 0
    pad_nops:  int = 0   # discarded: padding with no purpose (see the parser)
...
def _stats_comment_lines(stats: PairingStats, label: str, rule_breakdown: bool = False) -> list[str]:
    ...
    if stats.pad_nops:
        # Reported on its own line, never folded into the pair count: these are
        # a claim about padding we do not need, not about pairing we achieved.
        lines.append(f"#   padding nops: {stats.pad_nops} discarded "
                     f"(not counted as instructions or packets)")
    if rule_breakdown:
...
def annotate_output(fn_packets: list[tuple], annotate_liveness: bool = False) -> str:
    ...
    for fn_name, packets in fn_packets:
        # A worker reports its discarded-padding total as a bare entry rather
        # than a function, so it survives the parallel split without needing a
